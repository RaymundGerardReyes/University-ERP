...
// Domain Models reflecting Higher Education Reference Models (Ed-Fi / LIS)
export interface LMSActivity {
  id: string;
  type: 'Module' | 'Lesson' | 'Assignment' | 'Quiz' | 'Discussion';
  title: string;
  score?: number;
  maxScore?: number;
}
...
export interface Assessment {
  id: string;
  type: 'Prelim' | 'Midterm' | 'Final' | 'Practical';
  score?: number;
  maxScore?: number;
...
const generateMockData = (): DegreeProgram => {
  const years = [
  ...
  const getSubjectsForSem = (yearIdx: number, semIdx: number) => {
    const subs: CourseOffering[] = [];
    for (let i = 0; i < 8; i++) {
  ...
  const academicYears: AcademicYear[] = years.map((y, yIdx) => ({
    id: `ay-${yIdx}`,
    yearName: y.name,
    level: y.level,
...
export const useMyEnrollments = () => {
  const [data, setData] = useState<DegreeProgram | null>(null);
