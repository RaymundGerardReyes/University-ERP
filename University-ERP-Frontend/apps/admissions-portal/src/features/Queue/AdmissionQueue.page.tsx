...
export const AdmissionQueuePage: React.FC = () => {
  ...
  const groupedApps = useMemo(() => {
    ...
    applications.forEach((app: any) => {
      ...
      if (stage === 'Pending Intake' || stage === 'SecretaryQueue') {
        groups.Inbox.push(app);
      } else if (stage === 'DeanEndorsement' || stage === 'ChairpersonQueue') {
        groups.Review.push(app);
      } else if (stage === 'ExamScheduled') {
      ...
      <div style={{ display: 'flex', gap: 'var(--space-6)', overflowX: 'auto', paddingBottom: 'var(--space-4)' }}>
        ...
        <div style={{ minWidth: '320px', flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          {groupedApps.Inbox.map((app, idx) => (
            <Card
              ...
              onClick={() => alert(`Opening details for ${app.applicantName || app.name}`)}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ fontWeight: 600 }}>{app.applicantName || app.name}</span>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                  {new Date(app.submittedDate || app.submittedAt || Date.now()).toLocaleDateString()}
                </span>
              </div>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                {app.program || app.department || 'N/A'}
              </div>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <Badge colorScheme="warning">{app.stage || app.status || 'Pending'}</Badge>
              </div>
            </Card>
          ))}
        </div>
        ...
        <div style={{ minWidth: '320px', flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          {groupedApps.Review.map((app, idx) => (
            <Card
              key={app.id || idx}
              style={{ padding: '1rem', borderLeft: '4px solid var(--warning-text)', cursor: 'pointer' }}
              onClick={() => alert(`Opening details for ${app.applicantName || app.name}`)}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ fontWeight: 600 }}>{app.applicantName || app.name}</span>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <Badge colorScheme="info">{app.stage || app.status}</Badge>
              </div>
            </Card>
          ))}
        </div>
        ...
        <div style={{ minWidth: '320px', flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          {groupedApps.Exam.map((app, idx) => (
            <Card
              key={app.id || idx}
              style={{ padding: '1rem', borderLeft: '4px solid var(--info-text)', cursor: 'pointer' }}
              onClick={() => alert(`Opening details for ${app.applicantName || app.name}`)}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ fontWeight: 600 }}>{app.applicantName || app.name}</span>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <Badge colorScheme="warning">{app.stage || app.status}</Badge>
              </div>
            </Card>
          ))}
        </div>
        ...
        <div style={{ minWidth: '320px', flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          {groupedApps.Decision.map((app, idx) => (
            <Card
              key={app.id || idx}
              style={{ padding: '1rem', borderLeft: '4px solid var(--success-text)', cursor: 'pointer' }}
              onClick={() => alert(`Opening details for ${app.applicantName || app.name}`)}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ fontWeight: 600 }}>{app.applicantName || app.name}</span>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <Badge colorScheme="success">{app.stage || app.status}</Badge>
              </div>
            </Card>
          ))}
        </div>
