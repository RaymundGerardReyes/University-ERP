...
vi.mock('@university-erp/api-clients', () => ({
  admissionsApi: {
    getProgramCatalog: vi.fn().mockResolvedValue([
      { id: 'prog-1', degree: 'B.S.', major: 'Computer Science' }
    ]),
...
describe('ApplicationWizard Feature', () => {
  ...
  it('TC03: ApplicationWizard_Should_Render_Step1_ProgramSelection_By_Default', async () => {
    renderComponent();
    await waitFor(() => {
      expect(screen.getByText(/Step 1: Program Selection/i)).toBeInTheDocument();
      expect(screen.getByText(/Select Intended Program/i)).toBeInTheDocument();
    });
  });

  it('TC04: ApplicationWizard_Should_Prevent_Next_Step_If_Program_Not_Selected', async () => {
    renderComponent();
    await waitFor(() => {
  ...
  it('TC05: ApplicationWizard_Should_Progress_Steps_And_Submit_Payload', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Step 2: Academic History/i)).toBeInTheDocument();
    });

    await user.type(screen.getByPlaceholderText(/High School or College Name/i), 'Springfield High');
