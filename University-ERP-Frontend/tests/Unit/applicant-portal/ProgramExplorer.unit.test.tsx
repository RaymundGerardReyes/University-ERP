...
const mockGetSubjectCatalog = vi.fn();
const mockGetCurriculumByProgram = vi.fn();

vi.mock('@university-erp/api-clients', () => ({
  admissionsApi: { getProgramCatalog: () => mockGetProgramCatalog() },
  registrarCurriculumApi: {
    getSubjectCatalog: () => mockGetSubjectCatalog(),
    getCurriculumByProgram: (code: string) => mockGetCurriculumByProgram(code)
  }
}));
...
describe("ProgramExplorer Feature", () => {
  it("TC06: ProgramExplorer_Should_Display_Available_Programs_From_Catalog", async () => {
    ...
    mockGetSubjectCatalog.mockResolvedValue([
      { id: 'CS101', code: 'CS101', title: 'Intro to CS', units: 3, department: 'CCS', status: 'Active' }
    ]);
    mockGetCurriculumByProgram.mockResolvedValue({
      curriculumId: 'cur-1',
      programCode: 'BSCS',
  ...
  it("TC07: ProgramExplorer_Should_Display_Accountancy_BSA_Curriculum_When_View_Curriculum_Clicked", async () => {
    const queryClient = new QueryClient();
    mockGetProgramCatalog.mockResolvedValue([
      { id: 'BSA', degree: 'B.S.', major: 'Accountancy', college: 'College of Business and Accountancy', duration: '4 Years', intake: 'Fall / Spring' }
    ]);
    mockGetCurriculumByProgram.mockImplementation((code: string) => {
      if (code === 'BSA') {
        return Promise.resolve({
          curriculumId: 'cur-bsa',
          programCode: 'BSA',
              ...
              semesters: [
                {
                  ...
                  totalUnits: 20,
                  subjects: [
                    { subjectId: 's-acc101', code: 'ACC101', title: 'Financial Accounting and Reporting 1', units: 3, department: 'Accountancy', subjectType: 'Core', isElective: false, prerequisiteCodes: [] }
                  ]
                }
              ]
            }
    ...
    await waitFor(() => {
      expect(screen.getByText('B.S. in Accountancy')).toBeDefined();
    });

    const viewButton = screen.getByRole('button', { name: /View Curriculum/i });
    fireEvent.click(viewButton);

    await waitFor(() => {
      expect(screen.getByText('Financial Accounting and Reporting 1')).toBeDefined();
      expect(screen.getByText(/173 units total/i)).toBeDefined();
    });
  });
