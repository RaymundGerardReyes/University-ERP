...
vi.mock('@university-erp/auth-sdk', () => ({
  useAuth: () => ({
    user: { id: 'STU-101', name: 'Alex Student', email: 'alex@edu' },
...
vi.mock('@university-erp/api-clients', () => ({
  ...
  financeApi: {
    ...
    createPaymentSession: vi.fn().mockResolvedValue({ checkoutUrl: 'https://gateway.example.com/checkout' }),
  },
  financeBillingApi: {
    generateCashToken: vi.fn().mockResolvedValue('CASH-9999'),
  },
  apiClient: {
    get: vi.fn(),
    post: vi.fn(),
...
describe('EnrollmentPayment Feature', () => {
  ...
  it('TC14: EnrollmentPayment_Should_Render_Assessment_Details_When_Unpaid_Invoice_Exists', async () => {
    renderComponent();
    await waitFor(() => {
      expect(screen.getByText(/Enrollment Payment/i)).toBeInTheDocument();
      expect(screen.getByText(/Assessment Details/i)).toBeInTheDocument();
      expect(screen.getByRole('button', { name: /Proceed to Checkout/i })).toBeInTheDocument();
    });
  });

  it('TC15: EnrollmentPayment_Should_Display_Success_When_Invoice_Is_Paid', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Payment Successfully Settled/i)).toBeInTheDocument();
    });
  });

  it('TC16: initiatePayment calls /finance/payment-sessions and returns checkout reference', async () => {
    const { apiClient } = await import('@university-erp/api-clients');
    const { enrollmentPaymentApi } = await import('../../../apps/applicant-portal/src/features/EnrollmentPayment/EnrollmentPayment.api');

    vi.spyOn(apiClient, 'post').mockResolvedValueOnce({
      data: {
        sessionId: 'SESS-2026-0099',
        checkoutUrl: 'https://checkout.novabank.internal/pay/sess-99'
      }
    } as any);

    const result = await enrollmentPaymentApi.initiatePayment({
      assessmentId: 'ASS-2026-01',
      applicantId: 'APP-2026-0042',
      amount: 1500,
      paymentMethod: 'ONLINE_GATEWAY'
    });

    expect(apiClient.post).toHaveBeenCalledWith('/finance/payment-sessions', {
      invoiceId: 'ASS-2026-01',
      applicantId: 'APP-2026-0042',
      amount: 1500,
      purpose: 'Admissions Downpayment'
    });
    expect(result.paymentId).toBe('SESS-2026-0099');
    expect(result.referenceNumber).toBe('https://checkout.novabank.internal/pay/sess-99');
    expect(result.status).toBe('PAYMENT_PENDING');
  });

  it('TC17: getPaymentStatus maps Paid/Completed session to PAYMENT_VERIFIED', async () => {
    const { apiClient } = await import('@university-erp/api-clients');
    const { enrollmentPaymentApi } = await import('../../../apps/applicant-portal/src/features/EnrollmentPayment/EnrollmentPayment.api');
    ...
    const result = await enrollmentPaymentApi.getPaymentStatus('SESS-2026-0099');

    expect(apiClient.get).toHaveBeenCalledWith('/finance/payment-sessions/SESS-2026-0099');
  ...
  it.todo("should accurately calculate and explicitly list any localized tax on the final invoice");
  ...
  it.todo("should allow an Admissions Manager to manually click \"Mark as Paid via Wire Transfer\"");
  it.todo("should enforce 3D Secure authentication flow if mandated by the European payment gateway");
  ...
  it.todo("should cleanly localize the decimal separators based on browser locale");
