...
describe('InterviewScheduling Feature', () => {
  ...
  it('TC09: InterviewScheduling_Should_Render_Calendar_When_Not_Scheduled', async () => {
    renderComponent();
    await waitFor(() => {
  ...
  it('TC10: InterviewScheduling_Should_Show_Confirmed_When_Scheduled', async () => {
    vi.mocked(admissionsApi.getApplicantJourney).mockResolvedValueOnce({
      ...
      programs: [],
      documents: [],
      timeline: [{ date: '2026-08-12', event: 'Interview Scheduled', detail: 'August 12 at 10:00 AM' }]
    });
    renderComponent();
    await waitFor(() => {
      expect(screen.getByText('Interview Confirmed')).toBeInTheDocument();
      expect(screen.getByText('August 12 at 10:00 AM')).toBeInTheDocument();
    });
  });
